...
The six scoring criteria and their default weights (optimized for iPSC applications) are: Safety (0.30), Feasibility (0.25), Complexity (0.20), Risk (0.15), Confidence (0.10), and Consequence (0.08). Safety scores aggregate three contributions into a single 0–1 value: DSB burden (DSB-free approaches receive 1.0; single-DSB approaches receive 0.6, with p53-activity penalties of -0.10 for iPSC contexts; dual-DSB approaches receive 0.3, with simultaneous-DSB and p53 penalties of -0.10 each, reflecting documented translocation risk^5^ and p53-mediated toxicity in iPSCs); p53-context modifiers; and a bystander penalty of *0.08 × bystander_severity* that captures off-target edits within the editing window. The bystander term is applied inside the safety dimension as of Fix #2 (2026-04-20, see Scoring-methodology updates below); prior to that refactor it lived inside the Consequence dimension, which produced a safety ceiling of 1.0 for every DSB-free modality. The Consequence dimension now captures splice-proximity effects only (0.05 per near-exon-boundary proximity event, capped at 0.3). All weights are normalized at scorer construction time (raw weights sum to 1.08 and are divided by the total).

To assess rank stability under weight uncertainty, v3 implements Monte Carlo sensitivity analysis using 10,000 Dirichlet-sampled weight permutations. For each permutation, a weight vector is drawn from a Dirichlet distribution centered on the default weights (concentration parameter = 10), TOPSIS is re-run, and the rank of each strategy is recorded. The output includes, for each strategy: the fraction of permutations in which it is top-ranked (rank stability), the mean rank across permutations, and the full rank distribution. This provides an explicit measure of how sensitive the recommendation is to weight assumptions --- to our knowledge, no existing CRISPR design tool provides this form of rank stability reporting. As of Fix #3 (2026-04-20) the command-line tool interprets the stability score against pre-committed thresholds: rank stability ≥ 0.80 is reported as a *robust* recommendation; the range [0.70, 0.80) is *stable* (single recommendation but noted uncertainty); and values below 0.70 are classified as *flip-sensitive*, in which case the CLI surfaces the runner-up modality, the TOPSIS-score gap, and a per-dimension tradeoff breakdown so the user can make an informed context-specific choice.
...
**Top-1 accuracy** (top-ranked strategy matches a preferred or acceptable label): **100.0%** (30/30) with Fix #1 applied (90.0% at v3 initial release). The three top-1 misses in the v3 initial release were HDR-required large-structural-deletion cases — DMD (180 kb), NF1 (70 bp), and FBN1 (8 kb) — where the pipeline recommended prime editing despite PE being technically incapable of producing deletions of that scale with a single-pegRNA PE2/PE3 architecture. Fix #1 (2026-04-18) introduced a hard capability gate: when the explicit `structural_span_bp` field of a variant, or the implicit reference/alternate allele length, exceeds `PE_MAX_EDIT_SPAN_BP = 50`, the PE strategy is emitted as a visibly rejected `Strategy` object with `modality_prior_score = 0`, and the TOPSIS ranker skips it. With Fix #1 active, all three large-deletion cases flip to HDR as the top-1 recommendation. The 50 bp threshold is conservative relative to published PE2/PE3 single-pegRNA bounds (≤44 bp insertions in Anzalone et al., *Nature* 2019; ≤50–80 bp deletions in the same regime) and is parameterizable via a module-level constant.

**Top-3 accuracy** (at least one preferred or acceptable strategy in the top 3): **100.0%** (30/30) with Fix #1 active.

**Rejection accuracy** (infeasible strategies from the reject label absent from top-ranked output): **96.7%** (29/30) with Fix #1 active. The single remaining miss is a compound heterozygous COL7A1 case whose top-1 selection ("Single-step Base Editing") addresses one of two pathogenic variants only — an issue flagged separately in a compound-heterozygous-completeness audit (COL7A1_AUDIT_2026-04-19.md §7 option C) and not addressed by the three fixes reported here.

**Strategy distribution** across the 30 benchmark cases (with Fix #1): base editing 20% (6/30), prime editing 67% (20/30), HDR 13% (4/30). Compared to v2, where prime editing was top-ranked in 29/30 cases and base editing in 0/30, the v3 multi-nuclease engine rescued six cases to base editing via ABE8e paired with broader-PAM nucleases (primarily enFnCas9) that placed the target within the expanded editing window, and Fix #1 further rescued the three large-structural-deletion cases to HDR.

**Comparison to v2.** At the v3 initial release the overall top-1 accuracy (90.0%) already improved on v2 (86.7%) by resolving one COL7A1 case via scoring changes unrelated to the multi-nuclease rescue (see COL7A1_AUDIT_2026-04-19.md for the detailed decomposition of this delta). Fix #1 lifts top-1 from 90.0% to 100.0% by resolving the remaining three HDR cases. Beyond accuracy, v3 provides a more diverse and informative set of recommendations: where v2 effectively defaulted to prime editing for nearly all cases, v3 identifies base editing as a viable or preferred option at loci where the biology supports it, and HDR as the appropriate choice for structural deletions.
...
3. **Apply weights**: v_ij = w_j * r_ij, where default weights are Safety = 0.30, Feasibility = 0.25, Complexity = 0.20, Risk = 0.15, Confidence = 0.10, Consequence = 0.08. The six raw weights sum to 1.08 and are normalized at scorer construction time.

4. **Determine ideal (A+) and anti-ideal (A-) solutions**: A+ = max_i(v_ij) for benefit criteria (Safety, Feasibility, Confidence, Consequence); A- = min_i(v_ij) for the same. Complexity and Risk are cost criteria (lower is better), so their ideal/anti-ideal assignments are reversed.

5. **Compute relative closeness**: D_i+ = sqrt(sum_j (v_ij - A_j+)^2), D_i- = sqrt(sum_j (v_ij - A_j-)^2), C_i = D_i- / (D_i+ + D_i-).

The Consequence dimension score is constructed as *1 − penalty + bonus*, clamped to [0, 1], where the penalty captures splice-proximity events (0.05 per near-exon-boundary proximity, capped at 0.3) and the bonus rewards truly clean designs (+0.03 when `num_dsbs == 0` and `bystander_severity == 0.0`). Prior to Fix #2 (2026-04-20) the penalty also included a `bystander_severity × 0.08` term; Fix #2 relocated that term into the Safety dimension (see Scoring-methodology updates below). Strategies are ranked by the TOPSIS closeness score C_i, with ties broken by safety score.

#### Scoring-methodology updates (2026-04)

Three post-release refinements have been applied to the v3 scoring pipeline and are each documented in a dedicated notes file in the repository root:

- **Fix #1 — hard capability gate for prime editing** (2026-04-18). Single-pegRNA PE2/PE3 is rejected when a variant's edit span exceeds `PE_MAX_EDIT_SPAN_BP = 50`, with the threshold justified against literature bounds on RT-template length (Anzalone et al., 2019; Anzalone et al., *Nat Biotechnol* 2022 for twin-prime extensions). Rejected PE strategies are emitted with `modality_prior_score = 0` and `rejection_reasons` populated, making the exclusion visible to downstream UI. Lifts benchmark top-1 from 90.0% to **100.0%** by resolving three HDR-required cases (DMD, NF1, FBN1). Full details in `FIX_NOTES_2026-04-18.md`.
- **Fix #2 — bystander penalty relocated to Safety dimension** (2026-04-20). The coefficient `BYSTANDER_SAFETY_COEF = 0.08` is applied inside `_score_safety` rather than `_compute_consequence_penalty`. The refactor removes a safety ceiling at 1.0 for DSB-free modalities: after Fix #2, PE (bystander = 0) keeps safety = 1.0 while ABE/CBE with typical bystander severity in the 0.3–0.7 range score between 0.94 and 0.98 on safety. All 30 benchmark case top-1 modalities are preserved ("same score, same order"). Full details in `FIX_NOTES_2026-04-20.md`.
- **Fix #3 — rank-stability surfacing at inference** (2026-04-20). Pre-committed thresholds classify each top-1 recommendation as *robust* (stability ≥ 0.80), *stable* (0.70 ≤ stability < 0.80), or *flip-sensitive* (stability < 0.70). In the flip-sensitive branch, the command-line tool surfaces the runner-up modality, the TOPSIS-score gap, and a per-dimension tradeoff breakdown (which dimensions each candidate wins on, with a 0.05 material-difference threshold). This is strictly an output refactor — the ranking algorithm is unchanged. Full details in `FIX_NOTES_2026-04-20_fix3.md`.
...
**Figure 5. Benchmark accuracy results.** (A) Overall accuracy at v3 initial release: top-1 = 90.0% (27/30), top-3 = 100.0% (30/30), rejection = 86.7% (26/30). With Fix #1 (hard PE capability gate, 2026-04-18) applied, top-1 lifts to **100.0% (30/30)**, top-3 remains 100.0% (30/30), and rejection improves to 96.7% (29/30). (B) Accuracy by variant category (v3 initial release): 100% top-1 for clean base editing, PE transversion, PE small indel, compound heterozygous, dual base editing, and edge cases. Large deletion cases show 0/3 top-1 (all recommend PE) but 3/3 top-3 at the initial release, and 3/3 top-1 after Fix #1. (C) [If plotted] Fix-attribution bar chart showing which fix resolved which failing case.
